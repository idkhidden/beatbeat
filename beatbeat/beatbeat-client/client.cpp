...
int main()
{
    ...
    {
        cout << "[ beatbeat ] connection failed\n";
        closesocket(clientsocket);
        WSACleanup();
        Sleep(2000);
        return 1;
    ...
    cout << "[ beatbeat ] connected to server!\n";
    while (true)
    {
        ...
        if (heartbeat > 0)
        {
            cout << "[ beatbeat ] heartbeat received!\n";

            int randomheartbeat = distr(eng);
            string response = to_string(randomheartbeat) + "\n";

            send(clientsocket, response.c_str(), response.size(), 0);
            cout << "[ beatbeat ] heartbeat sent!\n";
        }
        else
        {
            cout << "[ beatbeat ] server disconnected\n";
            Sleep(2000);
            return 1;
        }
